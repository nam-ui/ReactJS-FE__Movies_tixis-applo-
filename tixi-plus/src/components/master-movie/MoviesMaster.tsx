...
function MoviesMaster(props: Props) {
    ...
    const params = useParams<PageURLTYPE>();
    const existedParamsPage = params.page ?? 1;
    // console.log(existedParamsPage);
    ...
    const { loading, error, data, refetch } = useQuery(PAGINATION, {
        variables: {
            page: state.page,
    ...
    const [onChangeRemovie, setChangeRemovie] = React.useState('')
    //  React.useMemo(() => {
    //     return refetch()
    // }, [props.stateFormCreated, onChangeRemovie])
    ...
    // React.useEffect(() => {
    //     history.replace(`/master/movie/page=${state.page}`)
    //     if (loading === false && data) {
    //         setState(data.pagination);
    //     }
    // }, [state.page, props.stateFormCreated])
    ...
    console.log(error,history,refetch,onChangeRemovie);

    return (
        <React.Fragment>
            <Grid container className={classes.root} spacing={sizeGird} style={{ margin: '3% auto' }} >
                {data.pagination.movies.map((movie: MovieType) => {
                    return <MovieMaster key={movie.id} propMovie={movie} onChangeId={(idProps) => { props.onChangeId(idProps) }} onClickOpenDetailsMovie={(statusOpen) => props.onClickOpenDetailsMovie(statusOpen)} onRemoveMovie={(status) => setChangeRemovie(status) } />
                })}
\            </Grid>
...
export interface Props {
    ...
    stateFormCreated: MovieTypeCreate
}
