...
function MoviesMaster(props: Props) {
  ...
  const [, setChangeRemovie] = React.useState("");
  ...
  if (loading === true)
    return (
      <div id="loadding-and-error-data-resp">
        <svg
        ...
        {data.pagination.movies.map((movie: MovieType) => {
          return (
            <MovieMaster
              ...
              propMovie={movie}
              onChangeId={(idProps) => {
                props.onChangeId(idProps);
              }}
              onClickOpenDetailsMovie={(statusOpen) =>
                props.onClickOpenDetailsMovie(statusOpen)
              }
              onRemoveMovie={(status) => setChangeRemovie(status)}
            />
          );
        })}
        \{" "}
